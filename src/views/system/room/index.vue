<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column show-overflow-tooltip label="房间名称"  align="center">
        <template slot-scope="scope">
    ...
    <el-dialog v-bind="$attrs" :visible.sync="dialogTableVisible" v-on="$listeners" @open="onOpen" @close="onClose" :title="title">
      <el-form ref="payForm" :model="payFormData" :rules="payRules" size="medium" label-width="100px">
        <el-form-item label="房间名称" prop="paytype">
          <el-input v-model="payFormData.name" placeholder="请输入房间名称" clearable :style="{width: '100%'}">
          </el-input>
        </el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 删除
      handleDel(index,row) {
        this.$confirm('此操作将放永久删除该房间, 是否继续?', '提示', {
      ...
      update() {
        this.loading = true;
        this.$put('/room', this.payFormData).then((result) => {
      ...
      add() {
        this.loading = true;
        this.$post('/room', this.payFormData).then((result) => {
          if (result.message !== null) {
            Message.success(result.message);
          }
          this.close()
      ...
      delete(index, row) {
        this.$del('/room/' + row.id).then((result) => {
          if (result.message !== null) {
            Message.success(result.message);
          }
          this.fetchData();
