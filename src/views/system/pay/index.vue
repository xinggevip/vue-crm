<template>
  <div class="app-container">
    ...
    <el-button type="primary" size="small" @click="handleAdd">添加支付方式</el-button>
    ...
    <el-dialog v-bind="$attrs" :visible.sync="dialogTableVisible" v-on="$listeners" @open="onOpen" @close="onClose" :title="title">
      <el-form ref="payForm" :model="payFormData" :rules="payRules" size="medium" label-width="100px">
        <el-form-item label="支付方式" prop="paytype">
          <el-input v-model="payFormData.paytype" placeholder="请输入支付方式" clearable :style="{width: '100%'}">
          </el-input>
        </el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      update() {
        this.loading = true;
        this.$put('/paytype', this.payFormData).then((result) => {
      ...
      add() {
        this.loading = true;
        this.$post('/paytype', this.payFormData).then((result) => {
          if (result.message !== null) {
            Message.success(result.message);
          }
          this.close()
      ...
      delete(index, row) {
        this.$del('/paytype/' + row.id).then((result) => {
          if (result.message !== null) {
            Message.success(result.message);
          }
          this.fetchData();
